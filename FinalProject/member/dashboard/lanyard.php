...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
        ...
        // Upload the image data to the lanyard_payment table
        ...
        // Check if the account_id exists in the lanyard_payment table
        $sql = "SELECT * FROM lanyard_payment WHERE account_id = '$account_id'";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            // Update the existing record
            $sql = "UPDATE lanyard_payment SET payment_receipt = '$imageData' WHERE account_id = '$account_id'";
        } else {
            // Insert a new record
            $sql = "INSERT INTO lanyard_payment (account_id, status, payment_receipt) VALUES ('$account_id', 'Not yet paid', '$imageData')";
        }
